...
class Table1Controller extends BaseController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		$records = $db->get('table1', $limit, $fields);
		$data = new stdClass;
	...
	function import_data(){
		if(!empty($_FILES['file'])){
			...
			if(!empty($file_path)){
				$db = $this->GetModel();
				if($ext == 'csv'){
					$options = array('table' => 'table1', 'fields' => '', 'delimiter' => ',', 'quote' => '"');
					$data = $db->loadCsvData( $file_path , $options , false );
				}
				else{
					$data = $db->loadJsonData( $file_path, 'table1' , false );
				}
				if($db->getLastError()){
	...
	function view( $rec_id = null , $value = null){
		...
		$record = $db->getOne( 'table1', $fields );
		if(!empty($record)){
			render_json($record);
		}
		else{
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$rec_id = $db->insert('table1',$modeldata);
			if(!empty($rec_id)){
				render_json($rec_id);
			}
			else{
	...
	function edit($rec_id=null){
		...
		if(is_post_request()){
			$modeldata=transform_request_data($_POST);
			$db->where('id' , $rec_id);
			$bool = $db->update('table1',$modeldata);
			if($bool){
				render_json($rec_id);
			}
			else{
